...
Square * construct_square(char *filename)
{
    ...
    fscanf( fp, "%d", &squareSize);

    Square *sqptr;
    sqptr = malloc(sizeof(Square));
    (*sqptr).size = squareSize;

    (*sqptr).array = malloc(sizeof(int*) * squareSize * squareSize);
    const char s[2] = ",";
    char *token;
    for (int i = 0; i < squareSize; i++) {
        char str[ squareSize * 2];
        fscanf( fp, "%s", str);
 token = strtok( str, s);
 while ( token != 
# 74 "verify_hetero.c" 3 4
                 ((void *)0)
# 74 "verify_hetero.c"
                     ) {
            int col = 0;
